...
import { Building2, Calendar, Users, MapPin, User, Mail, Phone } from 'lucide-react';
...
export default function HotelBooking() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const newOrder = {
      ...
      email: email,
      phone: phone,
      item: `Hôtel: ${destination} (${guests} pers, ${checkIn} au ${checkOut})`,
  ...
  return (
    <div className="pt-24 min-h-screen bg-navy pb-20">
      <div className="w-full h-64 md:h-80 relative mb-12">
        <LazyImage 
          src={serviceInfo?.imageUrl || "https://images.unsplash.com/photo-1566073771259-6a8506099945?q=80&w=2070&auto=format&fit=crop"}
      ...
      >
        ...
        <span className="text-[12px] uppercase tracking-[4px] text-gold mb-4 block">Réservation d'hôtel</span>
        ...
        <p className="text-text/90 max-w-2xl mx-auto uppercase tracking-widest text-sm drop-shadow">
          {serviceInfo?.content || serviceInfo?.description || "Trouvez les meilleurs hôtels pour votre séjour."}
        </p>
      </motion.div>
        ...
        {submitted ? (
           <div className="text-center py-12">
             ...
             <p className="text-sm text-text/70 uppercase tracking-widest leading-relaxed">
               Merci {name}. Votre demande de réservation pour {destination} a bien été enregistrée.
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                placeholder="Ex: Dubaï, Paris, Kinshasa..."
                className="w-full bg-navy border border-text/20 p-3 text-text focus:outline-none focus:border-gold transition-colors block"
              />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] text-text/90 uppercase tracking-widest flex items-center gap-2">
                <Users size={14} className="text-gold" /> Voyageurs
              </label>
              <select
                value={guests}
            ...
            <div className="space-y-2">
              <label className="text-[10px] text-text/90 uppercase tracking-widest flex items-center gap-2">
                <Calendar size={14} className="text-gold" /> Date d'arrivée
              </label>
              <input
                type="date"
                required
            ...
            <div className="space-y-2">
              <label className="text-[10px] text-text/90 uppercase tracking-widest flex items-center gap-2">
                <Calendar size={14} className="text-gold" /> Date de départ
              </label>
              <input
                type="date"
                required
      ...
      <Reviews serviceTitle="Réservation d'hôtel" />
</motion.div>
      </div>
</div>
  );
